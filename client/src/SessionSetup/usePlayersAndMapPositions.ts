...
export function usePlayersAndMapPositions(initialPlayerCount = 6) {
  const [mapPositions, setMapPositions] = useState<{ name: string; color: string | null}[]>(MAP_POSITIONS.slice(0, initialPlayerCount))
  const [players, setPlayers] = useState([...Array(initialPlayerCount).keys()].map(playerIndex => `Player ${playerIndex + 1}`))

  const handlePlayerCountChange = useCallback((_, newValue) => {
        ...
        diff < 0
          ...
          : [...Array(newValue).keys()].map(
              (playerIndex) => {
                ...
                const colorTaken = mapPositions.some(mp => mp.color === defaultMapPosition.color)
                const nameTaken = mapPositions.some(mp => mp.name === defaultMapPosition.name)

                return {
                  name: nameTaken ? `${defaultMapPosition.name} (copy)` : defaultMapPosition.name,
